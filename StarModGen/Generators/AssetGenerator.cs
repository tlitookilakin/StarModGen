...
namespace StarModGen.Generators
{
	...
	public class AssetGenerator : IIncrementalGenerator
	{
			...
			var assetLoads = ctx.SyntaxProvider.ForAttributeWithMetadataName<AssetLoad>("StarModGen.Lib.AssetLoadAttribute",
				...
				static (ctx, cancel) =>
				{
					...
					return new(
						...
						ctx.TargetSymbol.Name,
						((string)args[0].Value!).MakeLocal(),
						((IMethodSymbol)ctx.TargetSymbol).ReturnType.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat)
					);
				}
			);

			var assetIncludes = ctx.SyntaxProvider.ForAttributeWithMetadataName<AssetInclude>("StarModGen.Lib.AssetIncludeAttribute",
				static (node, cancel) => node is MethodDeclarationSyntax m && node.HasAnnotations("StarModGen.Lib.AssetEntryAttribute"),
				static (ctx, cancel) =>
				{
					var args = ctx.Attributes[0].ConstructorArguments;
					return new(
		...
		private record struct AssetProperty(string Type, string Prop, string PropType, string Asset, string? Local, string access);
		...
		private record struct TemplateData(AssetEntryMethod Entry, IReadOnlyList<AssetProperty> LocalProps, IEnumerable<AssetGroup> Assets);
		private record struct AssetInclude(string Owner, string Target, string Source);
		private record struct AssetEdit(string Owner, string Method, string Target);
		private record struct AssetLoad(string Owner, string Method, string Target, string Return);
		private record struct ConstantData(string ModId);
		private record struct AssetGroup(string Target, IList<AssetProperty> Props, IList<AssetEdit> Edits, IList<AssetLoad> Loads, IList<AssetInclude> Includes);
		...
		)
		{
			...
			foreach (var p in props)
			{
				...
				if (!Groups.TryGetValue(p.Asset, out var g))
					Groups.Add(p.Asset, g = new(p.Asset, [], [], [], []));
				g.Props.Add(p);
			}
			...
			foreach (var e in edits)
			{
				...
				if (!Groups.TryGetValue(e.Target, out var g))
					Groups.Add(e.Target, g = new(e.Target, [], [], [], []));
				g.Edits.Add(e);
			}
			...
			foreach (var l in loads)
			{
				...
				if (!Groups.TryGetValue(l.Target, out var g))
					Groups.Add(l.Target, g = new(l.Target, [], [], [], []));
				g.Loads.Add(l);
			}
			...
			foreach (var i in includes)
			{
				...
				if (!Groups.TryGetValue(i.Target, out var g))
					Groups.Add(i.Target, g = new(i.Target, [], [], [], []));
				g.Includes.Add(i);
			}
